...
def new_issue(repo, candidate):
    ...
    body = (f"<!-- kanji-hour-review:{run_id}:{info['version_code']} -->\n"
            ...
            f"[Review release]({url}/actions/workflows/review-release.yml) → "
            f"запуск **Review APK candidate {run_id}**.\n\n"
            f"Номер сборки — `candidate_run_id`: `{run_id}`; вводить его вручную не нужно.\n\n"
            "Откройте **Review deployments** и выберите окружение `release`. "
            "Для **Approve and deploy** сверьте SHA-256 с файлом рядом с установленным APK "
            "и добавьте комментарий:\n\n"
            f"```text\nSHA-256: {info['apk_sha256']}\n"
            "<Модель телефона, версия ОС и фактический результат проверки>\n```\n\n"
            "Для **Reject** достаточно причины отказа; этот кандидат нельзя будет принять повторным запуском. "
